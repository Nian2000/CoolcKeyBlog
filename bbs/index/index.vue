		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :up="upOption">
			...
			<view class="cu-card case isCard">
				<view class="cu-item shadow" v-for="(item, index) in list" :key="index" @click="goDetail(item._id)">
					<view class="image">
						<image class="sglist-imglist-img imageBorder" mode="widthFix" :src="item.imgList[0].imgurl"></image>
						<view class="cu-bar bg-shadeBottom">
							<text class="text-cut">{{ item.title }}</text>
						</view>
					</view>
					
					<view class="padding-sm flex align-center" style="width:100%;"><view class="price margin-right">{{ item.price }}</view><view class="text-sm text-cut">{{ item.content }}
